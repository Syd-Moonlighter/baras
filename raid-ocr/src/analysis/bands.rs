...
pub(super) fn name_text_span(slot: &CapturedImage, band: &Band) -> (u32, u32) {
    ...
    match left {
        ...
        //
        // Widening this to the Lanczos-3 radius was measured and is worse, in
        // two separate ways. A third column on the left reaches back into the
        // border this function just rejected, and recognition reads it as a
        // leading letter (`TORHAFNI` became `TTORHAFNI` in nine of forty
        // dumps). A third on the right costs the faintest marks: more
        // background in the crop widens the range `stretch_contrast` maps from,
        // and `J'ZIA VEL` loses its apostrophe. Two is the measured optimum.
